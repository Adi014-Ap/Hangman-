...
import random
import os
from ai_agent import AIGuesser
...
class HangmanGame:
    ...
    def setup_gui(self):
        self.main_frame = tk.Frame(self.root, bg="#ccffcc", bd=10)  # light green
        ...
        self.ai_guess_button = tk.Button(button_frame, text="AI Guess", command=self.make_ai_guess,
                                         font=("Arial", 18, "bold"), bg="#8B4513", fg="white", padx=20, pady=10)
        self.ai_guess_button.grid(row=0, column=3, padx=10)
    ...
    def make_guess(self):
        ...
        self.ai_agent.inform_of_guess_result(guess, is_correct, self.get_display_word())

    def make_ai_guess(self):
        if self.wrong_guesses >= 6 or all(letter in self.guessed for letter in self.word):
            return

        ai_guess = self.ai_agent.make_guess(self.get_display_word())
        if ai_guess:
            self.entry.delete(0, tk.END)
            self.entry.insert(0, ai_guess)
            self.make_guess()
        else:
            messagebox.showinfo("AI Turn", "AI has no more valid guesses!")
